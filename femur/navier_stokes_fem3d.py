...
def navier_stokes(data, force_arr):
    ...
    while len(noslip_elems) > 0:
        ...
        plane = sym.Poly(surface_from_three_points(*curr_noslip_nodes))

        #removing all nodes in plane
        nodes_in_plane = [x for x in data["nodes"] if is_on_plane(x, plane)]
        ...
        #     for node in data["connections"][elem]:
